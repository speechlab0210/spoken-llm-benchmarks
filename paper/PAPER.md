...
Results were extracted separately, in two stages. First, every catalogued benchmark's own paper was read. Second, the citation graph of all 360 benchmarks was harvested and the 1,619 citing papers on arXiv were read as well, since a benchmark is also run by teams other than the one that built it; 509 of those 1,619 (31 per cent) turned out to report results on a catalogued benchmark, the rest citing one only in prose. Surveys were excluded, so that no number is second-hand. The result is 15,220 sourced cells, each a (benchmark, model, metric, value, source) tuple copied from a published table, covering 197 models and 341 benchmarks with at least one cell, drawn from 686 distinct source papers. No number in the results tables was produced by running a model; every cell is a published claim by some paper, and cells from different papers on the same benchmark and model are stored separately rather than reconciled — a decision that turns out to be load-bearing for section 5.4.
...
Holistic multi-task suites number 64, 12 of them pre-2025, and their aggregate scores are the numbers most often quoted in model reports. Roughly a quarter of the catalogue's sourced result cells carry an aggregate-style metric name, and 185 of the 341 benchmarks with results report at least one. Among the 250 model-benchmark pairs reporting an aggregate plus four or more in-range components, the median component spread is 28.7 points. MMAU (2410.19168) with Qwen2.5-Omni reports, within a single source (2507.08128), a test-mini average of 71.5 over sound, music and speech splits spanning 65.9 to 78.1, a range of 12.2 points. SpeechR (2508.02018) with SALMONN reports an average of 34.73 over components ranging from 12.5 to 50.7. VoiceBench (2410.17196) aggregates components on incompatible scales, mixing 1-to-5 judge scores with 0-to-100 accuracies under a single average.
...
Adoption has not kept pace with production, though the shape of the shortfall is not the obvious one. Of 360 catalogued benchmarks, 341 carry at least one sourced result cell, so it is not true that most benchmarks go unmeasured. What is true is narrower and more telling: for 245 of those 341, every published number comes from the benchmark's own paper. Only 96 of 360 — just over a quarter — have ever been run by a team other than the one that built them, and only 30 have been taken up by five or more independent papers. A benchmark scored once, by its authors, at the moment of release is not yet an instrument the field is using; it is a proposal. Uptake is concentrated in a small set: MMAU appears in 94 independent papers, VoiceBench in 45, MMSU, WorldSense and MMAR in 42 each, AIR-Bench in 35. Model coverage is similarly thin — a median of six models per benchmark against a 197-model roster, leaving the matrix under a quarter dense. Nine models account for about 38 per cent of the 15,220 cells: Qwen2.5-Omni 1,327, Qwen2-Audio 880, Qwen3-Omni 678, Kimi-Audio 546, Audio Flamingo 3 544, SALMONN 500, Gemini 2.5 Flash 457, MiniCPM-o 449, GPT-4o-Audio 442. Seven of the nine are open-weight — the exceptions are the API-served Gemini 2.5 Flash and GPT-4o-Audio — which is the plausible explanation: comparability rests on what can be downloaded and re-run.
...
**Report per-task breakdowns alongside any aggregate, and never average incompatible scales.** The median component spread across the 250 aggregate-reporting model-benchmark pairs is 28.7 points; the aggregate is a claim about commensurability and should be defended as one.
...
Treat a single aggregate as a hypothesis about a per-task table that has not been shown. Ask whether a text-only baseline is reported; if it is not, read the number as an upper bound. Check whether a benchmark's numbers come from one source paper or several — for most benchmarks here, the answer is one. Note the date of the models being compared: the most-measured system in the catalogue is an early-2025 model and the second most-measured is a 2024 model, which means much of the comparability backbone is aged. And treat differences smaller than the reported per-task spread as not established, which for aggregate-reporting benchmarks means differences under roughly twenty-four points.
...
The most consequential result of that last genre is negative. A substantial share of reported spoken-LLM performance is recoverable without the audio; under deliberate audio-text conflict, models follow the text; scores move by up to a quarter under nuisance perturbations that have nothing to do with listening; aggregate scores conceal a median per-task spread of 28.7 points; and the same model on the same benchmark is published with materially different numbers by different papers. None of this is evidence that spoken LLMs do not work. It is evidence that the field does not currently know how well they work, because the dominant reporting format cannot distinguish a score earned by listening from a score earned by inference from the prompt.
