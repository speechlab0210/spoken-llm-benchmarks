...
| `data/crawl-ledger.json` | every paper the daily crawl has flagged since launch, with the reviewer's verdict and whether it entered the catalogue (generated by the build) |
| `data/rejected-2026-08-22.json` | the 120 candidates rejected on the build day, each with its reason and the part of the inclusion rule it failed |
| `data/inclusion-notes-2026-08-22.md` | the reader's verbatim notes from the build day: every borderline call, with the case for the other side |
...
- **How papers are found, and how one is judged to be a benchmark**, is written up with the count at every stage in the
  [Method section of the site](https://speechlab0210.github.io/spoken-llm-benchmarks/#method). The three files it cites —
  the build-day rejections, the reader's notes on borderline calls, and the ledger of everything the daily crawl has flagged —
  are in `data/` (see the table above).
